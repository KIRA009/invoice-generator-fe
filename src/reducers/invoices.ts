...
const invoices = createSlice({
    ...
    reducers: {
        ...
        editInvoice: (state: initialState, action: PayloadAction<Invoice>) => {
            let invoice = state.find(invoice => invoice.id === action.payload.id);
...
export const addInvoice = (payload: Invoice): AppThunk => async (dispatch) => {
    const id = (new Date()).getTime().toString();
    dispatch(invoices.actions.addInvoice({
        ...payload,
        id
    }))
}
...
export const editInvoice = (payload: Invoice): AppThunk => (dispatch) => {
    dispatch(invoices.actions.editInvoice({
        ...payload
    }))
}
